...
- Branch: `main`
- Current visible build: `v83.14`
- Local Owner page: use the Dock launcher or the active helper port near 8000; current working preview has been `http://localhost:8000/owner.html?v=83.14`.
...
- Latest checkpoint is `v83.14`: Owner imports now see Homebrew tools from GUI-launched helpers, use a remembered source selector whose `New...` option opens the native folder chooser, show one thumbnail/name row per import photo, and keep waiting counts aligned with processed/active/error rows.
...
- PhotosByElie active collaboration time is paused with `4:37` used as of 2026-05-24. Generated browser screenshots under `output/` and root `facebook-built-in-*` debug captures are local-only and ignored.
...
The import progress UI was polished in `v83.9`, `v83.10`, `v83.11`, simplified in `v83.13`, and count-corrected in `v83.14`: selected-folder imports no longer run banned-photo cleanup phases, import thumbnails are cached through the local helper, the sweep stack stays scoped to the current task, maintenance actions have explicit buttons, per-photo progress now shows one thumbnail/name row instead of step checkboxes, and waiting counts are derived from the same visible processed/error/active row model.
...
This refresh updates the durable summary/backlog/README/handoff/timelog state through `v83.14`, records the implemented import-source/maintenance UI, the `New...` folder-picker behavior, the simplified thumbnail/name import rows, and the corrected waiting-count math. No catalog count, Worker behavior, Stripe setting, or price behavior changed in this pass.
