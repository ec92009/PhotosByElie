...
- Public Expo catalog: `6,664` publishable media rows after the Italy catalog audit: AI/Leonardo `4,921`, France `315`, Italy `25`, Mexico `2`, Portugal `216`, Slovakia `2`, Spain `1,024`, USA `159`. Compared with the earlier `6,016`-row checkpoint at `736fe76b`, the catalog is `+648` rows overall; Italy was restored from `0` to `25` by adding Florence/Firenze, Pisa, San Gimignano, and Tuscany country hints.
...
- Latest checkpoint is `v86.1`; Owner Expo imports now skip known discarded/Waste Basket source paths, only show Owner-remembered source folders in the Expo pulldown, and auto-apply Green + 4-star selection only to Camera paths. AI/Leonardo stays tombstone-driven. Italy path/GPS inference is restored for Florence/Firenze, Pisa, San Gimignano, and Tuscany. The next import hardening pass should finish same-path newer re-export overwrite behavior instead of creating duplicate media rows.
...
1. **Finish import re-export de-duplication and clean today's duplicates.**
...
3. **Keep Owner/generated state handoff-ready.**
...
4. **Review checkout trust and buyer support wording.**
...
- It runs as automation id `pbe-daily-social-posts` at 09:00 local time. The run should pick three distinct themes, collect 5-10 watermarked public R2/social images for Facebook/Instagram and exactly 5 for Pinterest, publish only when Facebook/Instagram/Pinterest are already authenticated, and otherwise leave exact ready-to-publish captions, image lists, and manual steps. It must not use private masters, unwatermarked private renders, buyer downloads, Owner-only metadata, or secrets.
