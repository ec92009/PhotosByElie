...
- Owner-action JSON files are compatibility views, handoff files, audit files, or local config. `Owner.sqlite` is the local Owner source of truth; `assets/catalog/photosbyelie.sqlite(.br)` is the public catalog source of truth.
...
- On the localhost Title/Keywords review page, Owner can review the current proposal batch, single-click to select a row, double-click for detail, approve with `A`, reject with mutually exclusive horizontal reason checkboxes or `R`, propagate the current approve/reject decision plus reject note with `P`, and basket with the visible Block button or `H`/`X`. Reject reasons fill the note while keeping it editable, and previous reject notes load unchanged for further edits. Video rows show the usual play-triangle overlay on the preview. Rows autosave through the helper server; approved rows apply title/keyword values to generated catalog metadata/state files and add `Title_Keywords_Reviewed`, while rejected rows record rework state/comment in `Owner.sqlite`. The queue generator uses that stored rejection context to escalate rework attempts from local rules to the configured Codex model ladder, and records blocker/exhaustion details when a stronger proposal cannot be produced. Saved A/R rows remain visible during the current session and disappear after leaving/reloading; basketed rows disappear immediately. Source-file embedded metadata, public previews, private masters, and private render files are left untouched.
