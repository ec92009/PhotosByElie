...
Decision controls include an Approve checkbox plus visible horizontal, mutually exclusive reject-reason checkboxes: incorrect, use keywords, add details, use shoot, and other. Choosing any reject reason checks Reject internally, unchecks Approve, fills the reject note, leaves the note editable, and clears the other reject reasons. Existing previous reject notes must load unchanged for the Owner to edit or expand upon. Typing in the reject comment checks Reject and unchecks Approve. Clicking Approve does not erase the comment; it greys/read-onlys the comment until the Owner interacts with the comment again, which reactivates Reject.
