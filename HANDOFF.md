...
- In `v140.24`, the account panel keeps only the contextual Sign out control beside the signed-in identity. Signing out clears account-synced basket, likes, order references, and profile cache from the browser before visitor mode resumes, and Basket/Liked redraw on the same page so departed-account data is not left visible.
