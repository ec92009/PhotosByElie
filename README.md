...
- On localhost, `H` remains fixture-local while `X` sends a live-gallery photo to the recoverable Waste Basket through the Owner SQLite gateway; `U` restores the exact prior state, and `P` on the Waste Basket page puts a basketed photo back through the same lifecycle. Authenticated public Owner sessions can use the same grouped paths through the cloud Owner-action ledger and Max connector, including up to 500 selected gallery photos at once. Only a separately confirmed Empty Waste Basket action activates global tombstones; it retains source media, R2 objects, and history. Legacy R2 cleanup excludes gateway tombstones and remains a separately named repair path.
