...
- Sidecar has separate scheduler-facing maintenance tasks for Photos index sync and picked-only AI metadata planning. These do not appear in the Sidecar UI and can run on different LaunchAgent schedules; the current local defaults are 02:10 for Photos sync and 03:10 for picked-only AI planning. The AI planning task only includes picked rows in `unreviewed` or `rework` state; undecided/rejected/hidden/tombstoned/mock-uploaded items are intentionally excluded.
