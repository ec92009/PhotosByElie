...
- Public R2 sync and Saturn imports skip IDs and known source paths from Waste Basket and discarded tombstones, so rejected or owner-discarded photos are not reintroduced by later bulk uploads under a fresh selected-folder ID. AI/Leonardo imports stay tombstone-driven. Publish validation now fails if a discarded/tombstoned id leaks into the public catalog or `assets/expo-manifest.json`.
...
- `v94.2` adds the Etsy OAuth callback page plus local Etsy OAuth/API smoke-check helpers after Etsy approved the `photosbyelie-listing-publisher` integration. Etsy app credentials and OAuth tokens stay outside git under local environment/config only.
...
- Etsy API setup uses `scripts/etsy_oauth.mjs`, `scripts/etsy_api_check.mjs`, and `docs/sops/ETSY_API_SOP.md`. The current next step is to store the approved app keystring/shared secret outside git, run OAuth through `etsy-callback.html`, then start with draft/dry-run listing payloads from public catalog data and watermarked public previews only.
