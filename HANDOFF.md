...
- For another machine, commit/push the tracked public catalog/docs bundle first; sync ignored/private `Owner.sqlite` through the private Owner-state path if that machine needs the local Sidecar cleanup state.

- Next catalog action: commit/push the public catalog registration (`assets/catalog/photosbyelie.sqlite` plus `worker/photos-catalog.generated.mjs`) when ready, then verify public gallery counts and repaired previews after GitHub Pages updates.
