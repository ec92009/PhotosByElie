...
- Sidecar is now the planned local Apple Photos triage engine. Sidecar has its own visible local version, currently `v122.2` in `SIDECAR_VERSION`, and does not bump the public commercial site version by itself.
...
20. **Build Sidecar as the whole-library Apple Photos triage engine.**
   ...
   - Treat videos as first-class culling/editing items with media filters, local poster thumbnails, duration badges, and selected-item local playback before any forced iCloud materialization.
