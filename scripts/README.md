...
The generated product list currently includes digital file options, physical print sizes, and simple frame add-ons. Update both `export_photos_data.py` and `apply_curation_pass.py` when changing product ids, labels, prices, or availability thresholds so regenerated `photos-data.js` keeps the public checkout model intact.
