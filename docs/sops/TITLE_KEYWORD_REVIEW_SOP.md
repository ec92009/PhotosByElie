...
3. The generator must:
   ...
   - The default OpenAI ladder is `GPT-5.4 mini low → Luna Max vision → Sol High vision`: `gpt-5.4-mini`/`low`, `gpt-5.6-luna`/`max`, and `gpt-5.6-sol`/`high`. Every rung receives a bounded JPEG; `vision` is always `true` and is an input capability, not a provider model suffix.
   - Owner may save any positive number of ordered rungs. Each rung stores explicit `model` and `effort` strings; known GPT-5.4/5.6 combinations are validated before save, and Codex validates unfamiliar model identifiers at execution.
   - Every proposal records the requested model, resolved model, reasoning effort, `vision: true`, and the full ladder snapshot. `PBE_TITLE_KEYWORD_MODEL_LADDER`, `PBE_TITLE_KEYWORD_GENERATOR_MODEL`, `PBE_TITLE_KEYWORD_MODEL_RETRIES`, `PBE_TITLE_KEYWORD_MODEL_TIMEOUT_MS`, and `PBE_TITLE_KEYWORD_CODEX_BIN` may tune the generator for a run. Unless explicitly overridden, Backstage prefers Codex Desktop's bundled executable so Luna `max` is supported.
