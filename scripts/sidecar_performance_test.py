...
class PhotosBridgeRetirementTest(unittest.TestCase):
    def test_sidecar_server_has_no_standalone_bridge_lifecycle(self):
        source = (Path(__file__).resolve().parents[1] / "scripts" / "sidecar_server.py").read_text(encoding="utf-8")
        ...
        self.assertIn("request_library_index", source)
