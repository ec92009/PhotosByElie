...
const AUTH_BASE = process.env.META_AUTH_BASE || `https://www.facebook.com/${GRAPH_VERSION}/dialog/oauth`;
const CONFIG_DIR = path.join(os.homedir(), ".config", "photosbyelie");
const DEFAULT_TOKEN_FILE = path.join(CONFIG_DIR, "meta-token.json");
const DEFAULT_STATE_FILE = path.join(CONFIG_DIR, "meta-oauth-state.json");
...
function usage() {
  return `Usage:
  node scripts/meta_oauth.mjs --auth-url
  node scripts/meta_oauth.mjs --exchange-code <code>
  node scripts/meta_oauth.mjs --token-status

Environment:
  META_APP_ID          Required for --auth-url and --exchange-code.
  META_APP_SECRET      Required for --exchange-code.
  META_REDIRECT_URI    Required for --auth-url and --exchange-code.
  META_TOKEN_FILE      Optional. Defaults to ~/.config/photosbyelie/meta-token.json.
  META_SCOPES          Optional comma/space separated scopes for --auth-url.
...
function tokenFile() {
  return path.resolve(env("META_TOKEN_FILE") || DEFAULT_TOKEN_FILE);
}
...
function scopes() {
  const raw = String(args.get("scopes") || env("META_SCOPES") || "").trim();
...
async function graphGet(pathname, params) {
  ...
  if (!response.ok) {
    throw new Error(`Meta OAuth ${response.status}: ${JSON.stringify(payload)}`);
  }
  return payload;
}
...
async function exchangeCode() {
  ...
  if (!code) fail("--exchange-code requires the code returned by Meta.");
  const appId = requiredEnv("META_APP_ID");
...
async function main() {
  ...
  if (args.get("auth-url")) {
    console.log(JSON.stringify({
      ...
      scopes: scopes(),
      graph_version: GRAPH_VERSION,
      note: "Open authorization_url, approve Meta access, then run --exchange-code with the returned code. Tokens will not be printed.",
    }, null, 2));
    return;
  }
