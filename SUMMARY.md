...
- Title/keyword review queue state is SQLite-backed in ignored durable local `assets/owner-actions/Owner.sqlite`; the DB and WAL/SHM sidecars stay out of deployable GitHub Pages assets. Title/keyword batch JSON is compatibility/audit output and must not be treated as authoritative public catalog state.
