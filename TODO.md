...
- Real Estate client review is now organized as a focused wizard: multi-property clients start with property selection; media selection uses the full shared pool with click and Shift-click; titles can remove accidental picks; ordering uses one selected item per draggable row; output opens PDF and/or browser video previews.
- Real Estate outputs are scoped one property at a time. A single media item may be assigned to multiple projects, but the agent-facing wizard opens outputs for the active property.
...
5. **Finish Real Estate slideshow generation.**
   ...
   - Decide whether the browser preview stays HTML-only or also gains a client-side export path.
...
7. **Polish the Real Estate wizard.**
   - Keep each step compact and obvious on desktop and phone.
   - Make property switching, selected counts, range selection, title cleanup, and output previews feel calm for non-technical agents.
   - Continue improving drag ordering without hurting the phone workflow.
