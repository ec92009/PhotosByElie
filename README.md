...
- `real-estate.html`: private real-estate review workspace that loads an ignored client import bundle, supports album filtering, selection, title edits, and PDF batch JSON export
...
- `real-estate.js`: private client-gallery controller for Real Estate album filters, selected-photo draft state, editable PDF titles, batch JSON loading, and batch JSON export
...
- The private Real Estate review page loads the ignored local bundle at `tmp/real-estate-import/corine/app-context.js` on localhost by default, or any same-origin bundle passed with `?context=<path>`. It has a static client access gate, persists selected photo IDs and edited PDF titles with the store keys emitted by the importer, can reload an older batch manifest, exports the cloud-PDF batch JSON expected by the Real Estate workflow, and can generate a selected-photo PDF directly in the browser.
