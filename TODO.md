...
- Sidecar has separate scheduler-facing maintenance tasks for Photos index sync and picked-only AI metadata planning. These do not appear in the Sidecar UI and now run through Codex Scheduled tasks so they can keep different schedules. The optional LaunchAgent installer remains as a local fallback only. The AI planning task only includes picked rows in `unreviewed` or `rework` state; undecided/rejected/hidden/tombstoned/bridge-queued items are intentionally excluded.
- Sidecar upload eligibility is shown as a right-side Upload Bridge thumbnail rail; bridge queueing removes simulated rows from that rail, hides them from active Culling/Review windows, and warns when Owner's current R2 object state already covers planned private-master or public-preview keys. Sidecar metadata staging strips Owner keyword-blacklist terms before storing local keyword decisions, reading SQLite first with a JSON compatibility fallback when needed.
