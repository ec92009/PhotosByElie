...
3. The generator must:
   ...
   - The default OpenAI ladder is `Free → Luna Max vision → Sol High vision`: `gpt-5.4-mini`/`low`/text, `gpt-5.6-luna`/`max`/image, and `gpt-5.6-sol`/`high`/image. `vision` is an input capability, not a provider model suffix.
   - Every proposal records the requested project alias, resolved model, reasoning effort, vision flag, and ladder snapshot. `PBE_TITLE_KEYWORD_MODEL_LADDER`, `PBE_TITLE_KEYWORD_GENERATOR_MODEL`, `PBE_TITLE_KEYWORD_MODEL_RETRIES`, `PBE_TITLE_KEYWORD_MODEL_TIMEOUT_MS`, and `PBE_TITLE_KEYWORD_CODEX_BIN` may tune the generator for a run.
