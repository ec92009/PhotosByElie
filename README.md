...
- Current visible version: `v86.10`
...
- `real-estate.html`: private real-estate product workspace that loads a public-safe client context on GitHub Pages or an ignored local import bundle on localhost, starts with produced deliverables, supports create-new-product and edit-existing-product flows, click and Shift-click media selection from the full shared pool, selected-title cleanup, one-line drag ordering, view/download PDF/video outputs, selection-table resume, masked password entry, and selected-original ZIP delivery through the Worker
...
- `real-estate.js`: private client-gallery controller for the Real Estate product shelf, conditional property step plus four focused review pages, shared-pool media selection with range selection, property-scoped title/order state, view/download PDF/slideshow outputs, selection table loading/export, browser-built originals ZIP delivery, and legacy batch JSON loading
