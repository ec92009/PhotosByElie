...
- Sidecar is now the planned local Apple Photos triage engine. Sidecar has its own visible local version, currently `v125.3` in `SIDECAR_VERSION`, and does not bump the public commercial site version by itself.
...
- Sidecar Review is picked-item only and sorts oldest-to-newest for propagation. Title/keyword arrows propagate a single field through current-and-following picked rows in the same two-hour shoot window, while row Propagate carries metadata approval or additive AI rework category/comment guidance.
- Sidecar Review uses taller contained previews so portrait images can use the row's vertical space. The Upload Bridge rail is part of the default Sidecar workspace after a window loads and refreshes immediately after local approval/decision changes.
...
- Sidecar culling selection is tracked by stable Apple Photos asset IDs across local refreshes/refills/decision writes, reducing unexpected selection jumps when background UI work completes.
- Sidecar Review has a foreground `AI title pass` action for visible picked rows, while scheduled picked-only AI review remains the non-UI nightly path. Review AI rework guidance is additive: multiple category checkboxes can be staged together and their default notes combine with manual notes.
...
- Sidecar Culling/Review search applies server-side to the local Photos index across filenames, Apple Photos title/keywords, local decision metadata, seed keywords, and coarse location labels; changing or clearing search resets to the first matching window.
- Sidecar Culling has an explicit Refill window action that keeps the current working-set start anchored while scanning later Apple Photos rows to top up depleted visible space after Upload Bridge queueing, rejects, tombstones, or active filters hide rows. Date and index-start controls are intentionally absent from the UI.
...
- Sidecar has separate scheduler-facing maintenance tasks for Photos index sync and picked-only AI metadata planning. These do not appear in the Sidecar UI and now run through Codex Scheduled tasks so they can keep different schedules. The optional LaunchAgent installer remains as a local fallback only. The AI planning task includes picked rows that are not approved, including `unreviewed`, `rework`, and existing `proposed` rows; undecided/rejected/hidden/tombstoned/bridge-queued items are intentionally excluded. When the picked AI run needs visual evidence, use `python3 scripts/sidecar_maintenance.py picked-ai-preview-export` so previews come from the Photos Bridge app identity rather than raw Swift. Reviewed preview-backed proposals are written with `python3 scripts/sidecar_maintenance.py picked-ai-vision-propose --dry-run` followed by `python3 scripts/sidecar_maintenance.py picked-ai-vision-propose`, using `assets/owner-actions/sidecar-ai-metadata-vision-proposals-current.json` as the default input.
- Sidecar upload eligibility is shown as a right-side Upload Bridge thumbnail rail; bridge queueing removes simulated rows from that rail, hides them from active Culling/Review windows, and warns when Owner's current R2 object state already covers planned private-master or public-preview keys. Upload Bridge now blocks approved rows that still lack a safe gallery/country signal or have generic titles such as `2026`, `WhatsApp`, or `DJI Album`. Sidecar metadata staging strips Owner keyword-blacklist terms before storing local keyword decisions, reading SQLite first with a JSON compatibility fallback when needed.
- Upload Bridge is the planned materialization path for picked + Review-approved items. It should force Apple Photos downloads only for bridge-queued rows, export full assets, upload private masters plus public preview twins, and register the result through the Owner import/catalog path. It must not generate private JPG render triplets; those are lazy Worker cache objects created on demand at checkout/delivery time and retained after creation.
- Upload Bridge now has a local run ledger in SQLite and a one-item Apple Photos export dry run. The dry run writes `sidecar_upload_bridge_runs`/`sidecar_upload_bridge_run_items`, materializes one queued approved asset into `assets/owner-actions/sidecar-upload-runs/<run-id>/export/`, records planned private/public R2 keys and collisions, and still performs no R2 writes. Successful approved bridge uploads can be published to the public catalog with `python3 scripts/sidecar_maintenance.py register-uploaded-catalog`, which upserts Owner R2 object state and refreshes the generated Worker catalog.
- Existing private render triplets should be pruned from R2 as cache cleanup, except for sold/protected media. Missing render triplets are no longer an R2 repair gap; missing private masters remain the delivery blocker.
