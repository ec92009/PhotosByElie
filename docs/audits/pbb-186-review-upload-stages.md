...
Installed verification: **v265.2 / build 347**, source `bfe93d043b5a842ad76a3ec5fb768c51273dabb7`, signed with the existing Apple Development identity. Clean-source verification, bundled-runtime preview smoke, and strict installed signature checks passed. The idle app quit normally; previous build 346 is retained at `/Applications/.PhotosByElie Backstage.pre-347-20260922T201648.app`.

On the installed RE Marketing Review view, Picked+Uploaded with Approved and Hidden off returned 0; enabling Approved returned 156 approved photos, with Hidden still off. The installed inspector exposes the new Approve help and no separate Use After button. Pending-result rejection and After approval were verified with synthetic executable integration tests, without changing real-photo decisions. The user interacted with the app during the last filter toggle; further UI manipulation stopped. A read-only fixture state/receipt/version digest before and after installation and checks was identical: `e5f08d265325a07c5bde14320cf6c21d1a8da186c06b037976f688c7b68850eb`.

## Undismissable comparison recovery — build 348

Filtering out the photo behind a comparison sheet exposed a pre-existing error branch with no Close or Escape action. Normal Quit also failed while this sheet was open. With no active metadata or visual jobs, the exact Backstage process was terminated with SIGTERM and relaunched; Review recovered. No photo states were changed.

Installed **v265.3 / build 348**, source `cbed663d07e62f40a3b1e7992ddfb7a5433f098c`: the fallback now has Close and Escape, and the sheet target clears automatically when its photo leaves the visible Review IDs or the fixture changes. Targeted comparison regression test, clean signed build/runtime smoke and strict installed signature verification passed. Installed verification confirmed the blocked sheet was gone and Review's controls enabled. The missing-item fallback was source-checked; the original filter/compare race was not deliberately recreated on live photos. The workflow digest remains `e5f08d265325a07c5bde14320cf6c21d1a8da186c06b037976f688c7b68850eb`. Computer-use session reset after verification. Previous build retained at `/Applications/.PhotosByElie Backstage.pre-348-20260922T202405.app`.
