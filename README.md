...
- `scripts/new_owner_connector.py`: background Mac connector that polls authenticated cloud Owner actions and performs PhotoKit/Owner SQLite work without serving a localhost web UI; active work uses the configured short poll interval, while an idle connector backs off exponentially to one poll per minute. The Worker maintains a pending-action KV index so idle polling does not scan historical actions.
...
- On localhost, `H` or `X` sends a live-gallery photo to the Waste Basket through the durable hidden lifecycle state, `U` restores that state to active, and `P` on the Waste Basket page puts a basketed photo back through the same lifecycle. Authenticated public Owner sessions can now use the same `H`/`X` and grouped `U` path through the cloud Owner-action ledger and the Max connector, including up to 500 selected gallery photos at once. The connector records the lifecycle decision immediately without rebuilding the static catalog; the normal catalog publication pipeline consumes that durable state on its next run. Purging Waste Basket R2 copies promotes hidden rows to discarded, writes permanent tombstones, and queues R2 deletion. `D` is the stronger localhost-only discard action: it removes the photo from active catalog state, records discarded lifecycle state, writes `assets/discarded/discarded-photo-ids.json`, and queues R2 deletion for matching public previews, private masters, and private render JPGs.
- Owner now opens the recoverable Waste Basket through the local Mac connector.
  ...
  selected, permanently discard selected, and a separately confirmed Empty
  Waste Basket action. Permanent actions stay on the supported lifecycle and
  R2-deletion queue instead of editing SQLite directly.
