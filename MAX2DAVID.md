...
- The preferred proposal source is image vision: when public/local previews are available and credentials are configured, use a vision-capable AI pass that actually inspects the photo pixels plus current catalog metadata. Metadata/path heuristics are only a fallback, not the primary proposal mechanism.
...
- Repeat the Save approvals action at the bottom of the review page and include a back-to-top arrow in the page header.
