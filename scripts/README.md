...
On localhost, `real-estate.html` loads `tmp/real-estate-import/corine/app-context.js` by default. Pass `?context=<same-origin-app-context.js>` to review a different ignored client bundle. The page persists selected photo IDs and edited PDF titles with the store keys generated by the importer, can load a prior batch JSON, and can copy/download the next cloud-PDF batch manifest.
