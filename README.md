...
- The private Real Estate review page loads the ignored local bundle at `tmp/real-estate-import/corine/app-context.js` on localhost by default, or any same-origin bundle passed with `?context=<path>`. It persists selected photo IDs and edited PDF titles with the store keys emitted by the importer, can reload an older batch manifest, and exports the cloud-PDF batch JSON expected by the Real Estate workflow.
