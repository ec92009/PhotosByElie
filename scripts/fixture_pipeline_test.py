...
class FixturePipelineTest(unittest.TestCase):
    ...
    def test_review_approved_awaits_current_version_upload(self):
        ...
        self.assertEqual([x["assetId"] for x in fixture_review_window(self.root, "pending-upload", state_filters=["uploaded"])["items"]], [])
        excluded = fixture_review_window(self.root, "pending-upload", state_filters=["picked", "uploaded"], limit=1)
        self.assertEqual(excluded["items"], [])
        self.assertEqual(excluded["summary"]["total"], 0)
        self.assertFalse(excluded["hasNext"])
