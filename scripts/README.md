...
`export_photos_data.py` promotes a publishable catalog subset from the local import-cache manifest into the public SQLite catalog artifacts and leaves `photos-data.js` as a small bootstrap for existing static pages. It also writes the tiny homepage manifest to `home-data.js`. In the current GitHub-code/R2-media model, use `--external-media` so Git tracks metadata and public media keys rather than preview files. RAW-origin rows are kept out of public media because they do not have uploadable developed masters yet. Recoverable Waste Basket and active tombstone ids are kept out of public metadata; the PBB-79 gateway retains their source/R2 objects while fail-closing eligibility. Camera rows are also filtered by the same Green + 4-star eligibility policy used at import time; Apple Photo Albums and AI/Leonardo rows are not filtered by that Lightroom rule. Public R2 preview keys include photo `expo/<photo-id>_900.jpg` and `expo/<photo-id>_1800.jpg`, plus video `expo/<photo-id>_900.jpg` and `expo/<photo-id>_short_5s_720p.mp4`. Country/gallery origin stays in catalog metadata and `assets/media-sidecar.json`, not in the object key.
...
The hosted, fixture-frozen PBE Owner gallery is retired from normal Backstage
and customer use. The local-server command below is retained only for rollback
and historical contract tests; it is not an operator workflow:
...
For normal H/X/U/P review, no Apply step is needed: the localhost server updates review state immediately and rewrites the generated catalog/state files. H remains fixture-local; X enters the recoverable Waste Basket through the Owner SQLite gateway, U/P restore the exact prior state, and only a separately confirmed Empty Waste Basket action activates a global tombstone. Recoverable entries and active tombstones stay excluded from import/publication/search/delivery/commerce, while source and R2 media remain retained. Historical discard cleanup is legacy-only and excludes gateway tombstones. Unknown-to-country assignments are live server actions, not browser-staged assignments: they remove the assigned photo and its same-day cohort from Unknown immediately, update catalog metadata, write `Owner.sqlite`, and export `assets/owner-actions/country-assignments.jsonl` plus `assets/owner-actions/country-assignments.json` only as handoff/audit views. If the server update fails, the Unknown page should leave the card visible and reset the country selector.
...
The active storage contract is: Git tracks code/metadata and tiny assets; `tmp/import-cache` is the ignored disposable import/render workspace; `assets/owner-actions/reserve-data.json` remains only as localhost compatibility data; Waste Basket is the Owner review surface backed by `Owner.sqlite` entries, immutable provenance, operations, and receipts; public preview media belongs on R2/CDN. The old raw-first staging folders are retired.
...
The public site still exposes the same `window.photosByElieData` browser contract. Public pages load plain `assets/catalog/photosbyelie.sqlite` directly; normal catalog rebuilds do not generate or prefer Brotli-compressed SQLite. The ignored local `assets/owner-actions/Owner.sqlite` is the sole normal editable authority for identity, workflow, public metadata, collection membership, and publication state. The public SQLite file is its deterministic, privacy-safe projection and deployment artifact; tracked Owner JSON files are compatibility views or audit records, not state or write authorities. See the dated [PBE-173 Owner/catalog migration audit](../docs/audits/pbe-173-owner-catalog-migration.md).

The populated `photosbyelie.sqlite` schema keeps `media_items` dense by using short integer lookup ids for collections, cameras, lenses, media types, source origins, formats, asset types, and keyword terms. Rebuild it only through the supported Owner-authoritative projection path; `python3 scripts/build_public_catalog_db.py` and the legacy-named `node scripts/write_catalog_tsv.cjs` remain compatibility commands inside that guarded workflow. Current counts come from projection and deployment receipts, not this guide. `Owner.sqlite` has local workflow tables for settings, keyword blacklist, country assignments, title/keyword batches, queue state, proposals, and decisions. See `docs/architecture/sqlite-catalog-owner-state.md`.
...
`build_campaign_index.mjs` also validates optional YouTube campaign video metadata through `campaign-video.js`. A video is indexed and rendered only when its provider is YouTube, both supplied IDs have the exact YouTube ID shape, and `visibility` is explicitly `public`; the public campaign page derives privacy-enhanced embed and watch URLs instead of storing an arbitrary media URL or a duplicate MP4.
...
- IDs listed in historical owner discard tombstones are skipped for import/upload and may be handled by the separately named legacy `delete_discarded_r2_media.mjs` repair; PBB-79 gateway tombstones are excluded from that deletion path and retain source/R2 media
...
The wrapper sources `~/.zshrc`, pulls latest `main`, preflights import dependencies/source readiness, performs separately named legacy discard cleanup, imports selected media, regenerates catalogs/sidecars, backfills missing private render triplets, validates, commits, and pushes. The preflight phase checks Pillow for the sweep Python, `exiftool`, `sips`, `ffmpeg`/`ffprobe`, R2 upload backend readiness, and readable source folders before any photo work is queued. In automation mode it still imports the fixed Camera, Apple Photos, Leonardo, and configured Real Estate Saturn sources, while PBB-79 gateway tombstones remain retained and excluded from cleanup. In Owner UI mode, Start Expo import opens a local folder chooser and passes that folder to the wrapper as `--source-root`, so only that selected folder is scanned and the legacy cleanup phases stay out of the import dashboard. It uses `.review-logs/cloud-media-sweep.lock`; a scheduled automation will exit if a manual sweep is still active.
...
That legacy/repair command double-checks matching historical discarded public previews, private masters, and private JPG render objects in R2. It marks only those legacy keys as `marked_for_delete`, records successful idempotent delete calls back as `deleted_confirmed`, and keeps `assets/discarded-media-manifest.json` as a compatibility record. It explicitly excludes PBB-79 gateway tombstones. Routine runs trust Owner.sqlite; use `--deep-inventory` to refresh `current` R2 object state when storage volume looks suspicious. Legacy nested/private folders and country-prefixed public previews are retired; use `cleanup_legacy_r2_keys.mjs` only for an explicit one-time cleanup or audit of those old key families.
...
Legacy deletes also use the S3 backend when those credentials are present. The local Owner helper's normal Waste Basket Empty action does not delete R2 objects; only separately named historical cleanup may delete legacy keys, and it excludes gateway tombstones.
