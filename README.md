...
- Current visible version: `v78.32`
...
- `real-estate.js`: private client-gallery controller for Real Estate album filters, selected-photo draft state and drag ordering, editable PDF titles, selection table loading/export, browser-built originals ZIP delivery, and legacy batch JSON loading
...
- The Owner dashboard has a Real Estate client table backed by the localhost helper endpoint at `/__photosbyelie/real-estate-owner`. It saves client email addresses and plain-text local passwords in the ignored local settings file, derives username/source/gallery/prefix conventions from the client name, imports configured property folders from `/Volumes/Saturn/Pictures/RE/<Client>/<Property>`, publishes a sanitized public context under `assets/real-estate/<client>/app-context.js`, runs R2 upload dry-runs, can upload public previews plus private masters, and can prepare the `REAL_ESTATE_GALLERIES_JSON` Worker secret payload.
